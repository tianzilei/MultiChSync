...
def parse_offset_spec(spec: str) -> Dict[str, float]:
    ...
    if spec.endswith('.json'):
        try:
            ...
            raise ValueError(f"Failed to parse JSON offset file {spec}: {e}") from e
    ...
    for part in parts:
        ...
        if ':' in part:
            ...
            try:
                ...
                raise ValueError(f"Invalid offset value for device {device}: {offset_str}") from e
        else:
...
def parse_offset_list(spec: str) -> List[float]:
    ...
    if spec_stripped.startswith('['):
        try:
            ...
            raise ValueError(f"Failed to parse JSON offset string: {e}") from e
    ...
    for part in parts:
        ...
        try:
            ...
            raise ValueError(f"Invalid offset value: '{part}'. Expected a number.") from e
